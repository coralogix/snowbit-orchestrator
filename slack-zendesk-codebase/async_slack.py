...
async def handle_some_action(ack, body, logger, say):
    ...
    url = 'https://antstack.zendesk.com/api/v2/ticket_fields'
    data = requests.get(
        url,
...
async def create_zen_ticket(process_json, customer_name, slack_channel_handle_name):
    create_ticket = zenpy_client.tickets.create(
        Ticket(subject='{} | {} | {} | {} '.format(customer_name, process_json['aws_account_id'],
               ...
               requester=User(name=customer_name), assignee_id=5883758502685,
               priority='normal'
               )
...
async def main(process_json):
    ...
    ref_resp = dynamodb_client.get_item(
        TableName="refer_table",  # reference table aws_acc_id, cust_name, slack_channel, slack_workspace
        Key={
            'aws_account_id': {'N': str(process_json['aws_account_id'])}
        }
    )
